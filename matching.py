 ...
 #           print(0,x,total_pairs)
            ...
            if similarity > min(simU, simG):
                if ('univ' in keyword and 'univ' in x) and similarity > simU:
                    ...
                    pairs_k.append((keyword,x,similarity,dict_org[x], dict_id_country[dict_org[x]]))
...
def best_sim_score(clean_aff, light_raw, candidate_num, pairs_list, multi, simU, simG):
    ...
    pairs_list_reduced = [
        [item for item in pair if item[4] in country_check or item[4] in clean_aff.split()]
...
def Aff_Ids(input, dict_org, dict_mult, dict_city_ror, dict_country_ror, simU, simG, limit):
    
    """
    Matches affiliations in DataFrame 'DF' with names from dictionary 'dict_org' and their ROR_ids based on similarity scores.

    Args:
        m (int): The number of DOIs to check.
        DF (DataFrame): The input DataFrame containing affiliation data.
        dict_org (dict): A dictionary of names of organizations and their ROR_ids.
 ...
 #   print('KEYWORDS: ', keywords)
  ...
  #      print('try', s)
        try:
            pairs_k.append((s,s,1,dict_org[s],dict_id_country[dict_org[s]]))
 #           print('LUCKY')

           # pairs.append((s,s,similarity,dict_org[s], dict_id_country[dict_org[s]]))
 ...
 #   print('NEED CHECK PAIRS: ', pairs_check)
    ...
    #    print('OUTPUT BEST: ', best0)
        best1 = {x[0]:dict_org[x[0]] for x in best0 }
 ...
 #   print('MATCHED: ', matched_org)
    ...
    for org_list in best:
        org = org_list[0]
        conf = org_list[1]
        if dict_mult[org] == 'unique':
          #  print('unique:', org)
            if 'institu' in org and 'univ' in org:
                #print('both inst and univ', clean_aff)
                if dict_city_ror[org][0] not in clean_aff and dict_country_ror[org][0] not in clean_aff:
                    #print('pass')
                    pass
                else:
                    #print('correct')
                    id_list.append([org, conf, dict_org[org]])
            else:
                id_list.append([org, conf, dict_org[org]])

       
        else:
            if org in dict_city_ror:
                match_found = False

                for city in dict_city_ror[org]:
                ...
                if not match_found:
                    ...
                    for country in dict_country_ror[org]:
                    ...
                    if not match_found2:
                        ...
                        for sp in specific:
                            if sp in org:
                                id_list.append([org, conf, dict_org[org]])
                                spec_found = True
                    if not match_found2 and not spec_found:
               #         print('check active',  dict_city_ror[org])
                        
                        cand_ids = [x[1] for x in dict_city_ror[org]  if dict_status_new[x[1]][0] == 'active']
               #         print('cand_ids', cand_ids)
                        if len(cand_ids) == 1:
                            id_list.append([org, conf, cand_ids[0]])
